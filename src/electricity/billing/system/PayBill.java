...
public class PayBill extends JFrame implements ActionListener{
    ...
    public void actionPerformed(ActionEvent ae) {
        if (ae.getSource() == pay) {
            ...
            try {
                Conn c = new Conn();
                ResultSet rs = c.s.executeQuery("select total_bill from bill where meter_no = '"+meter+"' AND month='"+cmonth.getSelectedItem()+"'");
                if (rs.next()) {
            ...
            // Open Payment Gateway instead of old Paytm
            setVisible(false);
            new PaymentGateway(meter, cmonth.getSelectedItem(), amount);
        } else {
            setVisible(false);
        }
    }
